...
function createHandler(dependencies = {}) {
  const createService = dependencies.createService || ((onStage, onDatabaseDiagnostic) => { const values = config.read(); return service.create({ config: { read: () => values }, state, createDatabase: database.create, createTikTokClient: client.create, createPersistence: persistence.create, onStage, onDatabaseDiagnostic }); });
  ...
  return async function handler(req, res) {
    ...
    const diagnosticsEnabled = previewDiagnostic(); let stage = "database_initialization";
    ...
    try {
      const setStage = value => { stage = value; }; const result = await createService(setStage, diagnosticsEnabled ? setStage : undefined).start();
    ...
    } catch { if (diagnosticsEnabled) logStage(stage); return send(res, 404, { error: "affiliate_creator_runtime_unavailable" }); }
  };
}
