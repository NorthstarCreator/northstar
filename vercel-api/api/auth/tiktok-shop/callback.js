...
function createHandler(dependencies = {}) {
  ...
  const stateConsumer = dependencies.consumeTikTokShopOAuthState || consumeTikTokShopOAuthState;
  ...
  return async function handler(req, res) {
    ...
    if (!enabled()) {
      return response(req, res, 404, "authorization_disabled");
    }
    if (req.method !== "GET") {
    ...
    try {
      ...
      if (!(error instanceof TikTokShopOAuthStateError)) {
    ...
    if (!code || code === "null") return response(req, res, 400, "missing_callback_code");
