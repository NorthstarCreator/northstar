...
function testLifecycleScopeAndTimeRules() {
  ...
  for (const scope of APPROVED_CREATOR_SCOPES) initial(base({ grantedScopes: [REQUIRED_SCOPE, scope] }));
  for (const badScope of [[], ["seller.order.info"], ["creator.unknown"], ["partner.scope"], ["tiktok_display_api"]]) code(() => initial(base({ grantedScopes: badScope })), "invalid_authorization_scopes");
  ...
  code(() => initial(base({ accessTokenExpiresAt: NOW })), "invalid_credential_expiration");
  code(() => initial(base({ refreshTokenExpiresAt: NOW + 60 })), "invalid_credential_expiration");
  const withoutRefreshExpiry = base(); delete withoutRefreshExpiry.refreshTokenExpiresAt;
  assert.equal(initial(withoutRefreshExpiry).operation, "initial_authorization");
  for (const value of [String(NOW), NOW * 1000, NOW + 0.5, -1, Number.MAX_SAFE_INTEGER + 1, new Date()]) code(() => initial(base({ accessTokenExpiresAt: value })), "invalid_credential_expiration");
...
function testRevisionAndCredentialBoundaries() {
  ...
  for (const name of ["access_token", "refresh_token", "authorization_code", "code", "oauth_state", "app_secret", "client_secret", "sign", "signature", "raw_response", "provider_response", "request_id", "message"]) code(() => initial({ ...base(), [name]: "private-value" }), "invalid_persistence_input");
  ...
  for (const key of ["accessTokenCiphertext", "refreshTokenCiphertext", "grantedScopes", "accessTokenExpiresAt"]) code(() => invalidRefresh({ authorizationContext: context(), expectedRevision: 0, nextRevision: 1, state: "reauthorization_required", occurredAt: NOW, [key]: "private" }), "invalid_persistence_input");
}
