...
function create({ databaseUrl, execute, onDiagnostic } = {}) {
  ...
  return Object.freeze({ begin: value => one("select * from public.begin_affiliate_creator_authorization($1::text,$2::timestamptz,$3::timestamptz)", [value.stateDigest, value.expiresAt, value.occurredAt], ["connection_id", "authorization_revision", "expires_at"]), accept: value => one("select * from public.accept_affiliate_creator_authorization_callback($1::text,$2::timestamptz)", [value.stateDigest, value.occurredAt], ["connection_id", "authorization_revision", "credential_revision"]), account: () => one("select public.get_affiliate_creator_runtime_account_id() account_id", [], ["account_id"]), complete });
}
